...
export default function BoardPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Board header */}
      <div className="flex items-center justify-between px-5 py-3 border-b border-surface-200 bg-white/60">
        <div className="flex items-center gap-3">
          <h1 className="text-sm font-semibold text-surface-900">Board</h1>
      ...
      <div className="flex-1 flex gap-3 p-4 overflow-x-auto">
        {columns.map(([member, memberTickets]) => (
          <div
            ...
            className={cn(
              "flex flex-col w-64 shrink-0 rounded-xl bg-surface-100/60 transition-colors duration-150",
              draggingId && "ring-2 ring-accent-300 ring-dashed bg-accent-50/30"
            )}
          >
            {/* Column header */}
            <div className="flex items-center gap-2 px-3 py-2.5 border-b border-surface-200/50">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold text-surface-800 truncate">{member}</p>
              ...
              {(memberTickets as Ticket[])
                ...
                .map((ticket) => (
                  <div
                    ...
                    className={cn(
                      "bg-white rounded-lg border border-surface-200 px-3 py-2.5 cursor-grab active:cursor-grabbing transition-all duration-150",
                  ...
                  >
                    ...
                    <Link href={`/requests/${ticket.id}`} className="block">
                      <p className="text-xs font-medium text-surface-800 leading-snug line-clamp-2 hover:text-accent-600 transition-colors">
                        {ticket.title}
                      </p>
                    </Link>
